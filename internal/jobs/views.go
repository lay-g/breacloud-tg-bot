...
func RenderReport(day string, total int64, prevTotal int64, top []ServiceUsage, failed int) string {
	...
	if len(top) == 0 {
		...
		for i, u := range top {
			...
			fmt.Fprintf(&b, "%d. %s  %s\n", i+1, name, humanize.Bytes(u.Total))
		}
	}
	if failed > 0 {
...
func RenderTrafficAlert(alerts []TrafficAlert) string {
	...
	for _, a := range alerts {
		fmt.Fprintf(&b, "\n%s\n", a.Name)
		fmt.Fprintf(&b, "  已用 %s / %d GB（%d%%）", humanize.Bytes(a.UsedBytes), a.QuotaGB, a.Percent)
		if a.PeriodEnd != "" {
			fmt.Fprintf(&b, " · 周期至 %s", humanize.Date(a.PeriodEnd))
		}
		b.WriteString("\n")
	}
	return b.String()
...
func RenderExpiryAlert(items []ExpiryItem, days int) string {
	...
	for _, it := range items {
		fmt.Fprintf(&b, "\n%s\n", it.Name)
		fmt.Fprintf(&b, "  %s 到期（还有 %d 天）· %s", it.DueDate, it.DaysLeft, expirySuffix(it))
		b.WriteString("\n")
	}
	return b.String()
}
