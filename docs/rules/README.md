...
按问题领域分文件保存。每条记录包含四段：**现象**（可观察到的失败）、**原因**、**解决方案**、**相关文件**。

只记会再次踩到的东西——平台或依赖的反直觉行为、环境陷阱、必须绕过的限制。一次性笔误、纯代码 bug 不进这里。

## 索引

| 文件 | 领域 |
| --- | --- |
| [breacloud-api.md](breacloud-api.md) | BreaCloud 平台接口的实测行为与坑 |
